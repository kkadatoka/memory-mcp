...
// Parse environment thresholds robustly: use the environment value when provided
// and valid, otherwise fall back to the default numeric thresholds.
const ARCHIVE_THRESH: number = Number(process.env.ARCHIVE_THRESHOLD ?? 0.8) || 0.8; // 80% of max
const RETRIEVE_THRESH: number = Number(process.env.RETRIEVE_THRESHOLD ?? 0.3) || 0.3; // 30% of max
...
export class ConversationOrchestrator {
  ...
  private readonly ARCHIVE_THRESHOLD: number = ARCHIVE_THRESH; // Archive when 80% full
  private readonly RETRIEVE_THRESHOLD: number = RETRIEVE_THRESH; // Retrieve when 30% full
